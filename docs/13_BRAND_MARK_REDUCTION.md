...
Date: 2026-07-17 JST  
Status: **Superseded by Candidate 06 selection in `14_CANDIDATE_06_FORMAL_MARK.md`**
...
- Formal Markの形状を変更せず、その内側へ短い反射光の経路だけを追加
- 初回のみ、Deep Wine終端付近から弱い線状ハイライトが上方向へ一度移動
- 10秒の1サイクル中、可視時間は約1.4秒、最大不透明度22%。終端そのものは暗転・点滅しない
- ヒーロー領域だけに適用し、ヘッダー、名刺、小型アイコンには適用しない
- 移動後は赤い下端へ戻り、ループしない
...
- Animated Hero Markの一度だけの反射光とReduced Motionを確認する
- 旧トレース案が公開用アセットとして参照されていないことを確認する
- 公開前に問い合わせ導線、権利、商用利用条件、顧客情報を再確認する

## 11. Next-phase correction

追加の12案比較により、正式ロゴのベースをこの段階の形状から候補06へ変更した。正式ロゴと補助演出を分離する原則、独立した赤丸を使わない原則、白黒版を一つの面で成立させる原則は継承する。
