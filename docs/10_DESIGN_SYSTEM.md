...
Status: **Phase 04 provisional — monochrome Formal Mark / Hero motion comparison**
...
4. **One passing reflection** — ワインレッドは常設せず、Heroで一瞬通過する反射としてだけ使う。
...
| `--color-reflection-wine` | `#7A1F2B` | Heroアニメーション中だけ現れる反射の基準色 |
| `--color-reflection-peak` | `#A64050` | 反射の短い明部。常設しない |
...
:root {
  ...
  --color-maple: #C87532;
  --color-wood: #7A4526;
  --color-reflection-wine: #7A1F2B;
  --color-reflection-peak: #A64050;
  --color-red: #E33A2F;
  --color-muted: #AAA298;
  ...
  --motion-hero-reflection-visible: 1.8s;
}
```
...
- 中央が細く締まった縦長の黒い面
- 上下でわずかに異なる非対称カーブ
- 静止時は黒または生成り一色。ワインレッドの終端を常設しない

F字孔全体を波形へ変形しない。FrequencyはFormal Mark本体へ描き込まず、Animated Hero Markでのみ内側に沿う短い反射が周期的に下から上へ流れる挙動として示す。

### Three applications

| Variant | Role | Components | Use |
|---|---|---|---|
| Formal Mark 06 | 正式ロゴ | 黒または生成り一色のF字孔面 | 名刺、サイトヘッダー、ロゴタイプ、静止媒体 |
| Animated Hero Mark | 補助演出 | Formal Mark＋周期的に上へ流れる弱い反射 | Webヒーロー限定 |
| Small Icon 10 | 小サイズ候補 | 単色の角張った終端と大きなノッチ | favicon、SNSアイコン、16〜31px |
...
- Standard: black single-color silhouette
- Wood background: black single-color silhouette
- Light background: black single-color silhouette
- Dark background: Soft Ivory single-color silhouette
- Monochrome: 終端を含む一つの面。軌道や独立した点は追加しない
- Black-and-white print: 白地は黒面、黒地は白面
...
- Formal Mark: 常に静止。発光、軌道、点滅、初回フェードを加えない
- Animated Hero Markの反射: 10秒周期。そのうち約1.8秒だけ可視、残り約8.2秒は完全静止
- 反射の経路: F字孔の下端から内側のカーブに沿って上端へ一方向に移動し、上端で消える
- 反射色: 基準 `#7A1F2B`、短い明部 `#A64050`。静止時は両方とも不透明度0
- 最大不透明度: Wine 56%、Peak 32%、Ivory Glint 20%。ぼかしとグローを使用しない
- ループ境界: 可視区間終了後から次周期まで不透明度0を維持し、切り替わりを見せない
- 適用範囲: LPのヒーロー領域だけ。ヘッダー、名刺、常設ロゴ、小型アイコンでは使用しない
- UI hover / focus: 180ms
- Section reveal: 600ms以内
- Spotlight: 8〜12秒のごく小さな明度変化を候補とする
- `prefers-reduced-motion: reduce`では反射レイヤーを非表示にし、黒または生成り一色の静止ロゴだけを表示する
