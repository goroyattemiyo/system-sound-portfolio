...
Frequencyは外付け回路ではなく、F字孔そのものの流れとWeb上の赤い点の移動で示す。

### Minimum size

| Use | Recommended minimum | Absolute minimum |
|---|---:|---:|
| Digital symbol | 24px high | 16px favicon only |
...
シンボル上下端の大きい丸の直径を `x` とし、ロゴ外周へ最低 `1x`、推奨 `1.5x` の余白を確保する。名刺の断裁セーフティマージンとは別に確保する。

### Color variants

- Standard: Maple line + Signal Red point, transparent background
- Light background: Standardまたはblack monochrome
- Dark background: StandardまたはSoft Ivory reversed
- Monochrome: 全要素を同一色。赤い点も同色とする
- Black-and-white print: 白地は黒、黒地は白。赤を意味の必須条件にしない
