...
F字孔全体を波形へ変形しない。Frequencyは2.4pxの独立した軌道と、Web上で下端の赤が一度だけ移動する挙動で示す。
...
- Standard: black cutout + integrated Signal Red lower terminal + Soft Ivory track
...
- Monochrome: 終端を含む黒い面＋紙白の抜き軌道。独立した点は追加しない
...
- Signal travel: 下端から上端へ10.5秒で一度だけ移動。ループしない
