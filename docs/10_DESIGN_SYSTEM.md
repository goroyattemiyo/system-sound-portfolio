...
ユーザー提供の実物写真を形状資料としたトレースから、写真固有の細かな凹凸と説明的な要素を減らした。正式ロゴはFormal Mark、favicon・SNSアイコンはSmall Iconを使用する。Animated Hero MarkはWebヒーロー専用の演出版であり、正式ロゴのバリエーションとして印刷やヘッダーには使用しない。

- 上端の巻き込みと下端の涙滴状終端
- 中央で左右へ鋭く突き出す二つのノッチ
- 中央が細く締まった縦長の黒い面
- 上下でわずかに異なる非対称カーブ
- シルエット形状を保ったまま赤へ置換した下側の涙滴状終端
...
|---|---|---|---|
| Formal Mark | 正式ロゴ | 黒いF字孔面＋統合された赤い下端 | 名刺、サイトヘッダー、ロゴタイプ、静止媒体 |
| Animated Hero Mark | 補助演出 | Formal Mark＋白い軌道＋一度だけ移動する赤い信号 | Webヒーロー、ブランド紹介の演出面 |
| Small Icon | 小サイズ専用 | 太く整理したF字孔面＋統合された赤い下端 | favicon、SNSアイコン、16〜31px |
...
- Standard: black cutout + integrated Signal Red lower terminal
